...
/**
 * The type Schwarz pferd.
 *
 * @author Felix Mann
 * @version 1.0
 * @since 2019 -Dezember-26
 */
public class SchwarzPferd extends figur {
    private String bezeichnung = "sP";
    private static int[] xPos = new int[2];
    private static int[] yPos = new int[2];
    private static int cnt;

    /**
     * Instantiates a new Schwarz pferd.
     *
     * @param yPos the y pos
     * @param xPos the x pos
     */
    public SchwarzPferd(int yPos, int xPos) {
        if (cnt <= 2){
            bezeichnung += cnt + 1;
            SchwarzPferd.xPos[cnt] = xPos;
            SchwarzPferd.yPos[cnt] = yPos;
            cnt++;
        } else {
